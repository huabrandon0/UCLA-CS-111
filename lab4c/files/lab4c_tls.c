...
#include <poll.h>
#include <errno.h>
#include <openssl/ssl.h> //Note: use -lssl and -lcrypto to compile
...
int main(int argc, char *argv[])
{
	...
	ssl = SSL_new(ctx); //check NULL
	SSL_set_fd(ssl, sock_fd); //check == 0
	SSL_connect(ssl); //check != 1
	...
	{	
		...
		else if (poll_ret > 0)
		{
			...
			{
				rc = SSL_read(ssl, command_buf, BUF_SIZE - 1);
				if (rc == 0)
					break;
				else if (rc == -1)
				{
